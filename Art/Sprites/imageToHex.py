...
FOLDERNAME="\\Ordered\\" #folder containing images
...
def getAllImages():
	...
	for orderedFoldername in os.listdir(os.getcwd()+FOLDERNAME):
		# print("\n"+orderedFoldername)
		for filename in os.listdir(os.getcwd()+FOLDERNAME+"\\"+orderedFoldername+"\\"):
			# print("\n"+orderedFoldername)
			processImage(os.getcwd()+FOLDERNAME+"\\"+orderedFoldername+"\\"+filename,imageOutputs,keyOutput,orderedFoldername)
			# print(filename)
...
def processImage(imgName,imageOutputs,keyOutput,orderedFoldername):
	...
	trimmedName=orderedFoldername+"_"+trimImageName(imgName)
	...
	if SPLIT:
		...
		while imageSize[0]>=(CELLSIZE*(zones[0]+1)):
			...
			while imageSize[1]>=(CELLSIZE*(zones[1]+1)):
				...
				memLabel="t_"+trimmedName+"_"+str(zones[0])+"_"+str(zones[1])
				boundY=(32*(zones[1]+1))
		...
		newImage.save(os.getcwd()+"\\MapConverter\\"+orderedFoldername+"\\"+trimmedName+".png")
	else:
		x=y=0
...
def writeToFile(imageOutputs,pixels,x,y,memLabel,imageSizeX,imageSizeY,zoneX=0):
	...
	while y < imageSizeY:
		...
		while x < imageSizeX:
			...
			# tempHex=hex(a+r+g+b)
			if((x%32!=0 or not (SPLIT)) and x!=0):
				imageOutputs.write(", ")
